<a href="<?=$_SESSION['affiliateHome']?>"><img src="images/title-home.png" border="0"/></a><br />
...
$AfiliadoMenu = MenuAfiliado($db,$_SESSION['affiliateID'],$CategoriaS['id']);	
...
$AventurasAfiliado = MenuAfiliado($db,$_SESSION['affiliateID'],$Categoria['id']);

if($AventurasAfiliado > 0){	
...
$PlacesAfiliado = MenuAfiliado($db,$_SESSION['affiliateID'],$Categoria['id']);
...
$SpecialAfiliado = MenuAfiliado($db,$_SESSION['affiliateID'],$Categoria['id']);

if($SpecialAfiliado > 0){		  
	
	$sqlExiste = $db->consulta("SELECT * FROM `adventures` WHERE `categorie`='".$Categoria['id']."' AND `status` $sqlOption AND `special`= '1' Order by `order` ASC");
	  $existen = $db->num_rows($sqlExiste);
	if($existen==0){
...
//FIN DE MENU AFILAIDO

//Buscar Categoria en el Menu de Afiliado
function MenuAfiliado($db,$afiliado,$categoria){
	$sqlAfiliadoMenu = $db->consulta("SELECT * FROM `affiliates_menu` WHERE `affiliate`='".$afiliado."' AND `menu`='".$categoria."'");
