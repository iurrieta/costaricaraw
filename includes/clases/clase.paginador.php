 ...
 */
 ...
 if(!isset($_pagi_nav_anterior)){
	...
	$_pagi_nav_anterior = "&laquo; ".$lang['prev_reg'];
 } 
 
 if(!isset($_pagi_nav_siguiente)){
	// Si no se ha elegido una cadena para el enlace "siguiente"
	// Se toma la cadena por defecto.
	$_pagi_nav_siguiente = $lang['next_reg']." &raquo;";
 } 

 if(!isset($_pagi_nav_primera)){
	// Si no se ha elegido una cadena para el enlace "primera"
	// Se toma la cadena por defecto.
	$_pagi_nav_primera = "&laquo;&laquo; ".$lang['first_reg'];
 } 
 
 if(!isset($_pagi_nav_ultima)){
	// Si no se ha elegido una cadena para el enlace "siguiente"
	// Se toma la cadena por defecto.
	$_pagi_nav_ultima = $lang['last_reg']." &raquo;&raquo;";
 } 
...
$db = new MySQL(); 
 if($_pagi_conteo_alternativo == false){
 	...
 	$_pagi_result2 = $db->consulta($_pagi_sqlConta);
	// Si ocurrió error y mostrar errores está activado
 	...
 	$_pagi_totalReg = $db->GetResults($_pagi_result2,0,0);//total de registros
 }else{
	$_pagi_result3 = $db->consulta($_pagi_sql);
	// Si ocurrió error y mostrar errores está activado
	...
	$_pagi_totalReg = $db->num_rows($_pagi_result3);
 }
 ...
 */
 ...
 $_pagi_result = $db->consulta($_pagi_sqlLim);
 // Si ocurrió error y mostrar errores está activado
